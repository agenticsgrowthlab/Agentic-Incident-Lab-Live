...
Use the supplied database/screen context as the source of truth. Do not invent downstream acknowledgements, account status, or network outcomes.
...
Distinguish PAYMENT OUTCOME from OPERATIONS STATUS. A returned payment can be operationally resolved without becoming successful.
...
{context_text}""",
            ...
            findings.append({"name": name, "role": role, "finding": " ".join(finding.split())[:900]})
            usages.append(usage)
...
Prefer database evidence over inference. Never recommend retry/requeue when downstream acceptance/posting is ambiguous without first checking status.
A returned payment may be operationally resolved while remaining RETURNED.
Escalate to Incident Intelligence only for material/broad operational impact, sustained processing failure, or unresolved systemic risk.
...
{contract}""",
        ...
        usages.append(usage)
        lead = {"name":"ACH Operations Lead","role":"Orchestrator","finding":str(synthesis.get("why") or synthesis.get("summary") or "Team analysis complete.")[:900]}
        return {
